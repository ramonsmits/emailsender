			...
			try
			{
				...
				foreach(Dns_Answer answer in m_Answers){
					if(answer.QTYPE == QTYPE.MX){
						mx.Add(answer.RecordObj);
					}
				}

				MX_Record[] mxBuff = new MX_Record[mx.Count];
				mx.CopyTo(mxBuff);

				//---- Sort mx records by preference --------//
				if(mxBuff.Length > 1){
					int minPref = mxBuff[0].Preference;
					while(true){
						int cIndex = 0;					
						for(int i=0;i<mxBuff.Length;i++){
							MX_Record mxC = mxBuff[i];
							if(i > 0 && mxC.Preference < minPref){
								MX_Record mx1 = mxBuff[i-1];
								MX_Record mx2 = mxBuff[i];

								mxBuff[i-1] = mx2;
								mxBuff[i]   = mx1;

								minPref = mxC.Preference;
