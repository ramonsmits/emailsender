...
//		private int        m_SendTimeOut  = 30000;
	...
	//	private _LogWriter m_pLogWriter   = null;
		...
		public int NoOfActiveJob()
		{
			if(m_SendTrTable==null)
				return 0;
			else
				return m_SendTrTable.Count;
		}
		/// <summary>
		...
		/// <param name="trackID">An unique number to keep track of the current thread.</param>
		public void BeginSend(string[] to,string from,Stream message, string trackID)
		{	
			// Clear old errors
			m_pErrors.Clear();
			...
			m_Params.Add(t,new object[]{to,from,message,trackID});
			//t.Name = trackID; // DateTime.Now.Ticks.ToString() + t.GetHashCode().ToString();
			t.Start();
			//return t.Name;
		}
		...
		{
			if(!m_Params.Contains(Thread.CurrentThread))
			{
				return;
			}
								
			// Get params from Hashtable
			object[] param = (object[])m_Params[Thread.CurrentThread];

			string[] to    = (string[])param[0];
			string from    = (string)param[1];
			Stream message = (Stream)param[2];
			string trackID = (string)param[3];

			lock(m_Params)
			{				
				if(!m_Params.ContainsKey(Thread.CurrentThread))
				{
					return;
				}
				m_Params.Remove(Thread.CurrentThread);				
			}
			...
			foreach(string rcpt in to){
				...
				if (pos<0) pos = rcpt.Length;
				string eDomain = rcpt.Substring(pos);//*******
			...
			foreach(ArrayList sameServerEaddresses in rcptPerServer.Values){
				...
				t.Name = trackID; //Thread.CurrentThread.Name;
		...
		private void StartSendJob()
		{
			try
			{
				...
				// Raise event
				//OnNewSendJobStarted(Thread.CurrentThread.GetHashCode().ToString(),(string[])param[0],(int)param[3]);
				OnNewSendJobStarted(Thread.CurrentThread.Name,(string[])param[0]);
...
//						}
		...
		{			
			if(this.PartOfMessageIsSent != null){
				...
				string jobID = Thread.CurrentThread.Name;
				if(m_pOwnerUI == null){
		...
		protected void OnNewSendJobStarted(string jobID,string[] to)
		{
			if(this.NewSendJob != null){
				if(m_pOwnerUI == null){
					this.NewSendJob(this,new SendJob_EventArgs(jobID,to, m_SmartHost));
				}
				// For UI we must use invoke, because UI doesn't support multi Threading.
				else{
					m_pOwnerUI.Invoke(this.NewSendJob,new object[]{this,new SendJob_EventArgs(jobID,to,m_SmartHost)});
				}
			}
		}
		...
		protected void OnSendJobCompleted(string jobID,string[] to,ArrayList defectiveEmails)
		{
			if(this.SendJobCompleted != null){
				...
				if(m_pOwnerUI == null){
					this.SendJobCompleted(this,new SendJob_EventArgs(jobID,to,defEmails,m_SmartHost));
				}
				// For UI we must use invoke, because UI doesn't support multi Threading.
				else{
					m_pOwnerUI.Invoke(this.SendJobCompleted,new object[]{this,new SendJob_EventArgs(jobID,to,defEmails,m_SmartHost)});
				}
			}
		}
		...
		protected void OnError(SMTP_ErrorType type,string[] affectedAddresses,string errorText)
		{
			...
			lock(m_pErrors){
				m_pErrors.Add(new SMTP_Error(type,affectedAddresses,errorText, jobID));
			}

			if(this.Error != null){
				if(m_pOwnerUI == null){
					this.Error(this,new SMTP_Error(type,affectedAddresses,errorText, jobID));
				}
				// For UI we must use invoke, because UI doesn't support multi Threading.
				else{

					m_pOwnerUI.Invoke(this.Error,new object[]{this,new SMTP_Error(type,affectedAddresses,errorText, jobID)});
				}
			}
		}
