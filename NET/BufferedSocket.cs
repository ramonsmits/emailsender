		...
		public void SendData(Stream dataStream)
		{
			...
			if(m_pLogger != null){
				if(dataStream is MemoryStream && dataStream.Length < 200){
					...
					m_pLogger.AddSendEntry("Big binary, sended " + sendedCount.ToString() + " bytes.");
				}
			}
		}
			...
			if(state.ReadCount < state.MaxLength){
				...
				if(m_pLogger != null){
					if(state.Stream.Length < 200 && state.Stream is MemoryStream){
						...
						m_pLogger.AddReadEntry("Big binary, readed " + state.ReadCount.ToString() + " bytes.");
					}
				}
				...
				// We sended all data
					...
					if(m_pLogger != null){
						if(strm is MemoryStream && strm.Length < 200){
							...
							m_pLogger.AddSendEntry(m_pEncoding.GetString(ms.ToArray()));
						}
						else{
							m_pLogger.AddSendEntry("Big binary, readed " + strm.Position.ToString() + " bytes.");
						}
					}
